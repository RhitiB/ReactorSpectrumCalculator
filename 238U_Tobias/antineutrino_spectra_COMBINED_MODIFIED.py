import os
os.chdir(r"D:\Python_2\ENDF_FILE_READER\238U")
...
with open("Cum_Fission_238U.txt", "r") as input_file1:
...
with open("Extracted_Elements_Beta_Decay_data_1.txt", "r") as input_file2:
...
for key1 in dict1:    
    ...
    for key2 in (dict1[key1]):
        #print(dict1[key2][key3])
        endpoints_sublist=[] 
        for i in range(len(dict1[key1][key2])):
            if dict1[key1][key2][i]<511:
                E_e=[]
                for x in range(511,int(dict1[key1][key2][i]), -1):
                    E_e.append(dict1[key1][key2][i]-x)
                    #print(E_e)
            else:
                E_e=[]
                for x in range(511, int(dict1[key1][key2][i])):
                    E_e.append(dict1[key1][key2][i] - x)
...
#We are going to calculate the beta spectrum shape which is P#
#Modification is E_beta_max is the maximum kinetic energy#
#E_beta_max = E_o+E_beta here E_beta is E_e=m_e*c^2=511 KeV
###########Find the E_beta_max#################
E_beta_max_dict={}
for key1 in dict1:
    # print(dict1[key1])
    E_beta_max_dict[key1]={}
    for key2 in dict1[key1]:
        E_beta_max_sublist = []
        for i in range(len(dict1[key1][key2])):
           E_beta_max_sublist.append(dict1[key1][key2][i] + 511)
        E_beta_max_dict[key1][key2] = E_beta_max_sublist
#%%
spectrum_shape_dict = {}
for key1 in dict1:
    spectrum_shape_dict[key1] = {}  # Initialize a dictionary for each key in dict1
    for key2 in dict1[key1]:
        E_beta_max_value = E_beta_max_dict[key1][key2]
        E_anu_list = dict3[key1][key2]        
        spectrum_shape = []        
        for i in range(len(E_beta_max_value)):
            spectrum_shape_sublist = []
            for j in range(len(E_anu_list[i])):
                expr = ((E_beta_max_value[i] - E_anu_list[i][j]) ** 2 - (511)**2)
                if expr < 0:
                    continue
                P = float(math.sqrt(expr) * (E_anu_list[i][j] ** 2) * (E_beta_max_value[i] - E_anu_list[i][j]))
                if E_beta_max_value[i] <= 0:
                    continue
                spectrum_shape_sublist.append(P)            
            spectrum_shape.append(spectrum_shape_sublist)        
        spectrum_shape_dict[key1][key2] = spectrum_shape        
#%%
#print("///////////////////////////////////////////////////////////////////////////////////////////////////////////")
    ...
    for key2 in dict1[key1]:
        ...
        if key1 in cum_yield_dict and key2 in cum_yield_dict[key1]:
            ...
            if key2 in dictBR.get(key1, {}):
                ...
                P = spectrum_shape_dict[key1][key2]
                dn_dE_2 = []                
                if len(BR) == len(P):
...
# %%
plot_dir = "Subplots_238U_BetaDecay_Antineutrinospectra_1"
